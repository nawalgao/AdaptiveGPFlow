...
from .hetero_model import GPModelAdaptiveNoiseLengthscaleMultDim, GPModelAdaptiveLengthscaleMultDim
...
from .hetero_kernels import NonStationaryLengthscaleRBF
...
class GPMCAdaptiveLengthscaleMultDim(GPModelAdaptiveLengthscaleMultDim):
    ...
    def __init__(self, X, Y, kern, nonstat, num_latent=None): 
        ...
        X = DataHolder(X, on_shape_change='recompile')
        Y = DataHolder(Y, on_shape_change='recompile')
        GPModelAdaptiveLengthscaleMultDim.__init__(self, X, Y, kern, nonstat)
        for i in xrange(self.num_feat):
            self.kerns["ell"+str(i)] = self.kern_type
        self.V = Param(np.zeros((self.num_data, self.num_feat)))        # Lengthscales
        self.V.prior = Gaussian(0., 1.)
